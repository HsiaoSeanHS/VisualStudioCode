...
import uuid
import argparse
import yt_dlp  # replace youtube_dl import with yt_dlp
from youtube_upload import auth, lib, upload_video
import youtube_codec
...
def youtube_upload(args):
    upload_file_path = args.filename
    video_path = tempfile.mktemp(".mp4")
    # video_path = os.path.splitext(upload_file_path)[0] + ".mp4"
    youtube_codec.encode(upload_file_path, video_path, args.encrypt, args.key, args.video_fps)
...
def main(args):
    ...
    upload_parser.add_argument('--video-fps',
                               action='store',
                               metavar="video_fps",
                               default=20,
                               type=int,
                               help='set video fps, default value is 20')
    upload_parser.add_argument('--encrypt',
                               action='store_true',
                               default=False,
                               help='encrypt the file before uploading')
    ...
    retrieve_parser = subparsers.add_parser("retrieve", aliases=['r'], help="retrieve a video from YouTube save as <filename>")
    ...
    retrieve_parser.add_argument('--decrypt',
                                 action='store_true',
                                 default=False,
                                 help='decrypt the file after retrieving')
    retrieve_parser.add_argument('--key',
                                 action='store',
                                 metavar="decryption_key",
                                 default=None,
                                 help='decryption key, default is None')
